<template>
  ...
  <div class="demo-controls">
    <label>
      <input v-model="leftEnabled" data-testid="toggle-left-enabled" type="checkbox" />
      Left grid: allowCrossGridDrag
    </label>
    <label>
      <input v-model="rightEnabled" data-testid="toggle-right-enabled" type="checkbox" />
      Right grid: allowCrossGridDrag
    </label>
    <label>
      <input v-model="rightRejects" data-testid="toggle-right-rejects" type="checkbox" />
      Right grid: disableExternalDrop
    </label>
    <button data-testid="clear-log" type="button" @click="log = []">Clear log</button>
  </div>

  <div class="demo-cross-grids" data-testid="cross-grid-view-wrap">
    <div class="demo-grid-wrap">
      <p class="demo-grid-label">Left (layoutId: left)</p>
      <GridLayout
      ...
      >
        ...
        >
          <div class="demo-item">{{ item.i }}<small v-if="item.isStatic"> (locked)</small></div>
        </GridItem>
      </GridLayout>
    </div>
    <div class="demo-grid-wrap">
      <p class="demo-grid-label">Right (layoutId: right)</p>
      <GridLayout
      ...
      >
        <GridItem
          ...
          :data-testid="`grid-item-${item.i}`"
        >
          <div class="demo-item">{{ item.i }}</div>
        </GridItem>
      </GridLayout>
    </div>
  </div>
  ...
  <div class="demo-log" data-testid="event-log">
    <div v-if="log.length === 0">Drag an item between the two grids to see events here.</div>
