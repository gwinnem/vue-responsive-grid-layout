<template>
  <ExampleDemo title="Layout lifecycle events">
    ...
    <template #footer>
      <p v-if="lastBreakpoint">Last known breakpoint (read via a template ref): <code>{{ lastBreakpoint }}</code></p>
      <div class="event-log">
        <div v-if="events.length === 0" class="event-log__empty">Drag/resize an item, click one, or resize the window to see events appear here…</div>
