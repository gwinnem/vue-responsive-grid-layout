  ...
  <div class="container">
      ...
      <div class="col-sm">
        <form>
          <fieldset>
            ...
            <label class="hide" for="rowHeight">
              Row Height(px)
            </label>
            <input id="rowHeight" v-model="rowHeight" class="hide" type="number" />
            <label class="hide" for="colNum">
              Max Columns
            </label>
            <input id="colNum" v-model="colNum" class="hide" type="number" />
            <label class="hide" for="maxRows">
              Max Rows
            </label>
            <input id="maxRows" v-model="maxRows" class="hide" type="number" />
            <br class="hide" />
            <label class="hide" for="autosize">
              autosize
            </label>
            <input id="autosize" v-model="autoResizeGridLayout" class="hide" type="checkbox" />
            <label for="editMode">
              editMode
            </label>
            <input id="editMode" v-model="enableEditMode" type="checkbox" />
            <label class="hide" for="horizontalShift">
              horizontalShift
            </label>
            <input id="horizontalShift" v-model="horizontalShift" class="hide" type="checkbox" />
            <label class="hide" for="isBounded">
              isBounded
            </label>
            <input id="isBounded" v-model="isBounded" class="hide" type="checkbox" />
            <label for="isDraggable">isDraggable</label>
            <input id="isDraggable" v-model="isDraggable" type="checkbox" />
            <label class="hide" for="isMirrored">
              isMirrored
            </label>
            <input id="isMirrored" v-model="isMirrored" class="hide" type="checkbox" />
            <label for="isResizable">isResizable</label>
            <input id="isResizable" v-model="isResizable" type="checkbox" />
            <label class="hide" for="isResponsive">
              isResponsive
            </label>
            <input id="isResponsive" v-model="isResponsive" class="hide" type="checkbox" />
            <label class="hide" for="preserveAspectRatio">
              preserveAspectRatio
            </label>
            <input id="preserveAspectRatio" v-model="preserveAspectRatio" class="hide" type="checkbox" />
            <label class="hide" for="preventCollision">
              preventCollision
            </label>
            <input id="preventCollision" v-model="preventCollision" class="hide" type="checkbox" />
            <label class="hide" for="restoreOnDrag">
              restoreOnDrag
            </label>
            <input id="restoreOnDrag" v-model="restoreOnDrag" class="hide" type="checkbox" />
            <label class="hide" for="showCloseButton">
              showCloseButton
            </label>
            <input class="hide" id="showCloseButton" v-model="showCloseButton" type="checkbox" />
            <label class="hide" for="showGridLines">
              showGridLines
            </label>
            <input id="showGridLines" v-model="showGridLines" class="hide" type="checkbox" />
            <label class="hide" for="useBorderRadius">
              useBorderRadius
            </label>
            <input id="useBorderRadius" v-model="useBorderRadius" class="hide" type="checkbox" />
            <label class="hide" for="verticalCompact">
              verticalCompact
            </label>
            <input id="verticalCompact" v-model="verticalCompact" class="hide" type="checkbox" />
          </fieldset>
        </form>
      </div>
    </div>
    <div class="row">
      <div class="col-sm-2 hide">
        <div class="droppable-element" draggable="true" @drag="drag" @dragend="dragend">
      ...
      <div class="col-sm">
        <div class="layout">
          <div id="content" class="content">
            <GridLayout ref="refLayout" v-model:layout="testLayout" :auto-size="autoResizeGridLayout"
              :class="{ grid: showGridLines }" :col-num="colNum" :horizontal-shift="horizontalShift"
              :is-bounded="isBounded" :is-draggable="isDraggable" :is-mirrored="isMirrored" :is-resizable="isResizable"
              :margin="[10, 10]" :max-rows="maxRows" :prevent-collision="preventCollision" :responsive="isResponsive"
              :restore-on-drag="restoreOnDrag" :row-height="rowHeight" :use-border-radius="useBorderRadius"
              :use-css-transforms="true" :vertical-compact="verticalCompact" @columns-changed="colNumChanged">
              <GridItem v-for="item in testLayout" :key="item.i" :ref="el => setChildRef(el)" class="test"
                :enable-edit-mode="enableEditMode" :h="item.h" :i="item.i" :is-draggable="item.isDraggable"
                :is-resizable="item.isResizable" :is-static="item.isStatic" :min-h="item.minH" :min-w="item.minW"
                :preserve-aspect-ratio="preserveAspectRatio" :show-close-button="showCloseButton"
                :use-border-radius="useBorderRadius" :w="item.w" :x="item.x" :y="item.y"
                @container-resized="containerResizedEvent" @drag="dragEvent" @dragged="draggedEvent" @move="moveEvent"
                @moved="movedEvent" @remove-grid-item="removeGridItem" @resize="resizeEvent" @resized="resizedEvent">
                <span class="text">
                  {{ itemTitle(item) }}
                </span>
              </GridItem>
...
import { GridLayout, GridItem, type TLayoutItem } from 'vue-ts-responsive-grid-layout';
