<template>
  <ExampleDemo title="Configurable resize-hint appearance">
    ...
    <template #controls>
      <ExampleToggle v-model="showResizeHandles" label="showResizeHandles (grid default)" />
      <label class="example-controls-label">
        resizeHandleColor:
        <input v-model="resizeHandleColor" type="color">
      </label>
    </template>

    <GridLayout v-model:layout="layout" :resize-handle-color="resizeHandleColor" :row-height="100" :show-resize-handles="showResizeHandles">
      <GridItem :h="layout[0].h" :i="layout[0].i" :w="layout[0].w" :x="layout[0].x" :y="layout[0].y">
        <div class="example-item">grid default</div>
      </GridItem>
      <GridItem :h="layout[1].h" :i="layout[1].i" resize-handle-color="crimson" show-resize-handles :w="layout[1].w" :x="layout[1].x" :y="layout[1].y">
        <div class="example-item">own override (always visible, crimson)</div>
      </GridItem>
      <GridItem :h="layout[2].h" :i="layout[2].i" :show-resize-handles="false" :w="layout[2].w" :x="layout[2].x" :y="layout[2].y">
