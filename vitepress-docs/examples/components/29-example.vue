<template>
  <ExampleDemo title="compactNow, rearrange & duplicateItem">
    ...
    <template #controls>
      <button class="example-button" type="button" @click="scatter">Scatter items (leaves gaps)</button>
      <button class="example-button" type="button" @click="gridRef?.compactNow()">Tidy up (compactNow)</button>
      <button class="example-button example-button--secondary" type="button" @click="gridRef?.duplicateItem('a')">
        Duplicate item "a"
      </button>
    </template>

    <GridLayout ref="gridRef" v-model:layout="layout" :row-height="80" :compact-type="ECompactType.NONE" show-close-button>
      <GridItem v-for="item in layout" :key="item.i" :h="item.h" :i="item.i" :w="item.w" :x="item.x" :y="item.y" @remove-grid-item="removeItem">
