<template>
  <ExampleDemo title="Localizable ARIA strings">
    ...
    <template #controls>
      <ExampleToggle v-model="spanish" label="Use Spanish grid-wide labels" />
    </template>

    <GridLayout ref="gridRef" v-model:layout="layout" :aria-labels="gridAriaLabels" :row-height="80">
      <GridItem :h="layout[0].h" :i="layout[0].i" show-close-button :w="layout[0].w" :x="layout[0].x" :y="layout[0].y">
    ...
    <template #footer>
      <div class="aria-readout">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="row in ariaReadout" :key="row.id">
              <td>{{ row.id }}</td>
